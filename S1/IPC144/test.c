...
int main(void)
{
	...
	printf("(P1) %s is %d years old and has the phone %s and lives at %s, %s, %s, %s\n",
		...
		p1.address.street, p1.address.city, p1.address.province, 
		p1.address.postCode);
	...
	for (i = 0; i < numStudents; i++)
	{
		printf("(myClass[i]) %s is %d years old and has the phone %s and lives at %s, %s, %s, %s\n",
			myClass[i].personInfo.name, myClass[i].personInfo.age, myClass[i].personInfo.phone,
			myClass[i].personInfo.address.street, myClass[i].personInfo.address.city, myClass[i].personInfo.address.province,
			myClass[i].personInfo.address.postCode);

		printf("Marks: ");
		for (j = 0; j < myClass[i].numMarks; j++)
		{
			printf("%.2lf ", myClass[i].marks[j]);
		}
		printf("\n");
	}
