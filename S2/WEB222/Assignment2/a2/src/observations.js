...
function observationSummary(data) {
  for (let i = 0; i < data.results.length; i++) {
    ...
    console.log(
      `ID-${observation.id} "${observation.species_guess}" (Date: ${observation.observed_on_details.date})`
    );
  }
}
...
function observationsByGeoPrivacy(data, geoPrivacy) {
  ...
  for (const observation of data.results) {
    if (
      ...
      (geoPrivacy === null && observation.geoprivacy === null)
    ) {
...
function transformObservation(original) {
  return {
    ...
    coords: [Number(original.location.split(',')[1]), Number(original.location.split(',')[0])],
...
function transformObservations(data) {
  const result = [];
  data.results.forEach((observation) => {
    result.push(transformObservation(observation));
  });
  return result;
}
...
function getObservationsByPositionalAccuracy(data, options = {}) {
  ...
  return data.results.filter((observation) => {
    if (eq !== undefined) {
      return observation.positional_accuracy === eq;
    }
    if (gt !== undefined && lt !== undefined) {
      return observation.positional_accuracy >= gt && observation.positional_accuracy <= lt;
    }
    if (gt !== undefined) {
      return observation.positional_accuracy >= gt;
    }
    if (lt !== undefined) {
      return observation.positional_accuracy <= lt;
    }
    return true;
  });
}
...
function getPlaces(data) {
  return data.results.map((observation) => {
    ...
    observation.place_ids.forEach((placeId) => {
      placeIds[placeId] = `https://www.inaturalist.org/observations?place_id=${placeId}`;
    });
    return placeIds;
  });
}
...
function getUserStats(data) {
  ...
  data.results.forEach((result) => {
    totals.observations += result.user.observations_count;
    totals.journals += result.user.journal_posts_count;
    totals.species += result.user.species_count;
  });

  averages.observations = totals.observations / count;
  averages.journals = totals.journals / count;
  averages.species = totals.species / count;
...
function extractTimeZones(data) {
  let timeZones = [];
  data.results.forEach((result) => {
    if (!timeZones.includes(result.created_time_zone)) {
      timeZones.push(result.created_time_zone);
    }
    if (!timeZones.includes(result.observed_time_zone)) {
      timeZones.push(result.observed_time_zone);
    }
  });
  return timeZones;
}
...
function extractTimeZones2(data) {
  ...
  data.results.forEach((result) => {
    timeZones.add(result.created_time_zone);
    timeZones.add(result.observed_time_zone);
  });
